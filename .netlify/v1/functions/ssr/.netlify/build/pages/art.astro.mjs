...
const $$Art = createComponent(async ($$result, $$props, $$slots) => {
  ...
  mediums.unshift("All");
  const url = new URL(Astro2.request.url, Astro2.site);
  const mediumParam = url.searchParams.get("medium");
  const selectedMedium = mediumParam && mediumParam.trim() !== "" ? mediumParam : "All";
  function slugify(str) {
    return str.toLowerCase().replace(/\s+/g, "-");
  }
  const filteredArt = selectedMedium === "All" ? allArtworks : allArtworks.filter((art) => slugify(art.data.medium) === mediumParam);
...
}, "E:/Desktop/artist website/art-void/src/pages/art.astro", void 0);

const $$file = "E:/Desktop/artist website/art-void/src/pages/art.astro";
const $$url = "/art";
